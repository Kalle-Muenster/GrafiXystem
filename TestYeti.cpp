...
TestYeti::TestYeti(const char* objFile,const char* textureFile,bool addToSceneGraph)
	...
	conXtor->GetConnected<AudioEmitter>()->LoadeStream("testtrack8.mp3");
	conXtor->AddConnectable<MusicListener>();
	conXtor->GetConnected<MusicListener>()->SetThreshold(0,0.025f);
...
TestYeti::DoUpdate(void)
{
	...
	//	float* fftWin = (float*)AUDIO->GetMasterOutFFT();
